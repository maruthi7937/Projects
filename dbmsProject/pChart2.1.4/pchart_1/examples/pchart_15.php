<?php   
 ...
 /* database connection to retrieve data*/
							...
							$percentage_changed = array();
							$sample_id = array();
							$count=1;
							$con = mysqli_connect(HOSTNAME, USERNAME, PASSWORD, DBNAME) or die("connect failed");			                
							if($stmt = mysqli_prepare($con, "Select percentile_changed from fastq_masker")){
 ...
 $MyData = new pData();   
 $MyData->addPoints($percentage_changed,"ScoreA");    
 ...
 $myPicture->drawText(210,213,"3D pie charts",array("R"=>255,"G"=>255,"B"=>255));
 ...
 $myPicture->drawText(320,500,"percentage changed",array("DrawBox"=>TRUE,"BoxRounded"=>TRUE,"R"=>0,"G"=>0,"B"=>0,"Align"=>TEXT_ALIGN_TOPMIDDLE));
 $myPicture->drawText(640,500,"sample_id / percentage changed",array("DrawBox"=>TRUE,"BoxRounded"=>TRUE,"R"=>0,"G"=>0,"B"=>0,"Align"=>TEXT_ALIGN_TOPMIDDLE));
