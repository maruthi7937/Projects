					...
					<td>
	   				    	<?php
	  				     		echo "<table align='left' border='1'>";	     
			 	   	                ...
			 	   	                $name = $_POST["name2"];
							$category = $_POST["category021"];

							
							if($category == "all")
							{
								$line1 = "SELECT gi.gene_id, gi.gene_short_name  FROM gene_info gi WHERE NOT EXISTS(SELECT s.sample_id FROM sample s WHERE NOT EXISTS (SELECT g.sample_id FROM gene_fpkm g WHERE (g.sample_fpkm > ".$name.") AND (g.sample_id=s.sample_id) AND (g.gene_id = gi.id) ) );";
							}
							else
							{
								$line1 = "SELECT  gi.gene_id, gi.gene_short_name FROM gene_info gi WHERE (gi.id IN 
( SELECT g.gene_id FROM gene_fpkm g, sample s WHERE (g.sample_fpkm  > ".$name.") AND  (s.sample_id=g.sample_id) AND (s.sample_id = ".$category.")));";
